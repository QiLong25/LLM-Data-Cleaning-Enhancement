...
cat_dims, cat_idxs, con_idxs, X_train, y_train, X_valid, y_valid, X_test, y_test, train_mean, train_std, raha_detect_train = data_prep_openml(opt.dset_id, opt.dset_seed,opt.task, datasplit=[.65, .15, .2])
saveEmb = input("Do you want to save embeddings? ([y] for yes, others for no) ")
if saveEmb == 'y':
    print("Saving embeddings...")
    import csv
    fileX = open("EmbeddingX.csv", 'w', encoding='utf-8')
    fileY = open("EmbeddingY.csv", 'w', encoding='utf-8')
    csv_writerX = csv.writer(fileX)
    csv_writerY = csv.writer(fileY)
    csv_writerX.writerows(X_train['data'])
    csv_writerY.writerows(y_train['data'])
    fileX.close()
    fileY.close()
...
## Dataset and Loader Generation ##
train_ds = DataSetCatCon(X_train, y_train, cat_idxs,opt.dtask,continuous_mean_std)
trainloader = DataLoader(train_ds, batch_size=opt.batchsize, shuffle=False,num_workers=4)
raha_detect_loader = []
for batch in range(int(len(raha_detect_train) / opt.batchsize) + 1):
    if batch == int(len(raha_detect_train) / opt.batchsize):
        raha_detect_loader.append(raha_detect_train[batch * opt.batchsize : len(raha_detect_train)])
    else:
        raha_detect_loader.append(raha_detect_train[batch * opt.batchsize : (batch+1) * opt.batchsize])
...
if opt.dset_id == 1:                ## input by user!!!!!!!!!!!!!!!!!!!!!
    # y_dim = 54            # hospital
    y_dim = 100
...
if opt.pretrain:
    ...
    model = SAINT_pretrain(model, cat_idxs,X_train,y_train, continuous_mean_std, opt, device, raha_detect_loader)
    # print("model's data type: ", type(model))
    # preRecord = input("Do you want to save the pretrained model? ([y] for yes, others for no) ")
    # if preRecord == 'y':
    #     saving_path_name = "./pretrains/model1"
...
if opt.ssl_samples is not None and opt.ssl_samples > 0 :
    ...
    train_ds = DataSetCatCon(X_train, y_train, cat_idxs,opt.dtask,continuous_mean_std)
    trainloader = DataLoader(train_ds, batch_size=train_bsize, shuffle=False,num_workers=4)
